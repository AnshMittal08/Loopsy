...
export default function OnboardingCard({ storageKey, title, steps }) {
  ...
  const headingId = `onboarding-${storageKey}`;
  const close = () => {
      ...
      {open && (
        ...
        >
          <Motion.div
            ...
            className="relative w-full max-w-sm sm:max-w-md max-h-[90vh] overflow-y-auto rounded-2xl bg-surface-container-lowest border border-outline-variant/20 shadow-warm-xl p-6 sm:p-7 outline-none"
          >
            ...
            <h2 id={headingId} className="font-display display-wonk text-2xl font-bold text-on-surface mb-5">{title}</h2>
