...
import { compileChart } from "@/lib/engine/chart";
import { validatePattern } from "@/lib/engine";
import { createPattern } from "@/lib/models/patternModel";
import { generateId } from "@/lib/utils/helpers";
import { requireAuthenticatedUser } from "@/lib/auth/session";

// Colorwork chart (M4 "Draw"): a pixel grid → exact, verified flat pattern.
// Fully deterministic (no AI), so it isn't rate-limited.

function buildChartPattern({ name, yarnWeight, cols, rows, grid, difficulty }) {
  const out = compileChart({ name, yarnWeight, cols, rows, grid });
  if (!out.ok) {
...
export async function POST(request) {
  try {
    ...
    const { name, yarnWeight, cols, rows, grid, difficulty, stream } = body;
    if (!Array.isArray(grid) || !cols || !rows) {
      return NextResponse.json({ error: "A chart grid is required." }, { status: 400 });
    }

    if (!stream) {
      const pattern = buildChartPattern({ name, yarnWeight, cols, rows, grid, difficulty });
